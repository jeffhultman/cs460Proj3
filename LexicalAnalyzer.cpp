...
token_type LexicalAnalyzer::GetToken ()
{
	...
	if (input.fail())
	{
		...
		//p2file << '\t' << setw(16) << left << "EOF_T" << endl;
		return EOF_T;
	}
	...
	//p2file << '\t' << setw(16) << left << token_names[token] << lexeme << endl;
	return token;
}
...
void LexicalAnalyzer::ReportFunctionEntered (const string & function, token_type t)
{
  p2file << "Entering " << function << " function; current token is: " << GetTokenName(t) << ", lexeme: " << GetLexeme() << endl;
}

void LexicalAnalyzer::ReportFunctionExited (const string & function, token_type t)
{
  p2file << "Exiting " << function << " function; current token is: " << GetTokenName(t) << endl;
}

void LexicalAnalyzer::ReportRuleUsed (const string & rule)
{
